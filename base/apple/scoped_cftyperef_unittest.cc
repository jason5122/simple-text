...
TEST(ScopedCFTypeRefTest, ConstructionSameType) {
    ...
    ScopedCFTypeRef<CFMutableArrayRef> retain_scoper(array, OwnershipPolicy::kRetain);
    EXPECT_EQ(array, retain_scoper.get());
    EXPECT_EQ(2, CFGetRetainCount(array));

    ScopedCFTypeRef<CFMutableArrayRef> assume_scoper(array, OwnershipPolicy::kAssume);
...
TEST(ScopedCFTypeRefTest, ResetSameType) {
    ...
    retain_scoper.reset(array, OwnershipPolicy::kRetain);
    EXPECT_EQ(array, retain_scoper.get());
    EXPECT_EQ(2, CFGetRetainCount(array));

    ScopedCFTypeRef<CFMutableArrayRef> assume_scoper;
    assume_scoper.reset(array, OwnershipPolicy::kAssume);
    EXPECT_EQ(array, assume_scoper.get());
    EXPECT_EQ(2, CFGetRetainCount(array));
