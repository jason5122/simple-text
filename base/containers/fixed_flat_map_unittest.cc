...
//     constexpr auto kSquares = MakeFixedFlatMap<int, int>({{1, 1}, {2, 4}, {3, 9}, {4, 16}});
...
//     EXPECT_THAT(kSquares, ElementsAre(Pair(1, 1), Pair(2, 4), Pair(3, 9), Pair(4, 16)));
// }

// TEST(FixedFlatMapTest, MakeFixedFlatMap_UnsortedInput) {
//     constexpr auto kMap =
//         MakeFixedFlatMap<std::string_view, int>({{"foo", 1}, {"bar", 2}, {"baz", 3}});
//     static_assert(ranges::is_sorted(kMap), "Error: Map is not sorted.");
//     static_assert(ranges::adjacent_find(kMap) == kMap.end(),
//                   "Error: Map contains repeated elements.");
//     EXPECT_THAT(kMap, ElementsAre(Pair("bar", 2), Pair("baz", 3), Pair("foo", 1)));
// }
...
//         {1, {1}},
//     });
//     EXPECT_THAT(kSquares, ElementsAre(PairType(1, {1}), PairType(2, {4}), PairType(3, {9}),
//                                       PairType(4, {16})));
// }
