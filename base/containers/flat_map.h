...
class flat_map : public ::base::internal::flat_tree<Key, internal::GetFirst, Compare, Container> {
private:
    using tree = typename ::base::internal::flat_tree<Key, internal::GetFirst, Compare, Container>;
...
template <class K>
auto flat_map<Key, Mapped, Compare, Container>::at(const K& key) -> mapped_type& {
...
template <class Key, class Mapped, class Compare, class Container>
template <class K>
auto flat_map<Key, Mapped, Compare, Container>::at(const K& key) const -> const mapped_type& {
...
template <class Key, class Mapped, class Compare, class Container>
auto flat_map<Key, Mapped, Compare, Container>::operator[](const key_type& key) -> mapped_type& {
...
template <class Key, class Mapped, class Compare, class Container>
auto flat_map<Key, Mapped, Compare, Container>::operator[](key_type&& key) -> mapped_type& {
    iterator found = tree::lower_bound(key);
    if (found == tree::end() || tree::key_comp()(key, found->first)) {
...
template <class Key, class Mapped, class Compare, class Container>
template <class K, class M>
auto flat_map<Key, Mapped, Compare, Container>::insert_or_assign(K&& key, M&& obj)
    -> std::pair<iterator, bool> {
    auto result = tree::emplace_key_args(key, std::forward<K>(key), std::forward<M>(obj));
    if (!result.second) {
...
template <class Key, class Mapped, class Compare, class Container>
template <class K, class M>
auto flat_map<Key, Mapped, Compare, Container>::insert_or_assign(const_iterator hint,
                                                                 K&& key,
                                                                 M&& obj) -> iterator {
    auto result =
        tree::emplace_hint_key_args(hint, key, std::forward<K>(key), std::forward<M>(obj));
    if (!result.second) {
        result.first->second = std::forward<M>(obj);
    }
    return result.first;
...
template <class Key, class Mapped, class Compare, class Container>
template <class K, class... Args>
    requires(std::is_constructible_v<typename flat_map<Key, Mapped, Compare, Container>::key_type,
                                     K &&>)
auto flat_map<Key, Mapped, Compare, Container>::try_emplace(K&& key, Args&&... args)
    -> std::pair<iterator, bool> {
    return tree::emplace_key_args(key, std::piecewise_construct,
...
auto flat_map<Key, Mapped, Compare, Container>::try_emplace(const_iterator hint,
                                                            K&& key,
                                                            Args&&... args) -> iterator {
    return tree::emplace_hint_key_args(hint, key, std::piecewise_construct,
                                       std::forward_as_tuple(std::forward<K>(key)),
                                       std::forward_as_tuple(std::forward<Args>(args)...))
        .first;
}
...
constexpr flat_map<Key, Mapped, KeyCompare, Container> MakeFlatMap(
    ...
    std::ranges::transform(unprojected_elements, std::back_inserter(elements), proj);
    return flat_map<Key, Mapped, KeyCompare, Container>(std::move(elements), comp);
}
