...
//     friend bool operator<(const TemplateConstructor&, const TemplateConstructor&) { return false; }
// };
...
// using TypedTree =
//     flat_tree<typename ContainerT::value_type, std::identity, std::less<>, ContainerT>;
// using IntTree = TypedTree<std::vector<int>>;
// using IntPair = std::pair<int, int>;
// using IntPairTree = flat_tree<IntPair, std::identity, LessByFirst<IntPair>, std::vector<IntPair>>;
// using MoveOnlyTree = flat_tree<MoveOnlyInt, std::identity, std::less<>, std::vector<MoveOnlyInt>>;
// using EmplaceableTree =
//     flat_tree<Emplaceable, std::identity, std::less<>, std::vector<Emplaceable>>;
// using ReversedTree = flat_tree<int, std::identity, std::greater<int>, std::vector<int>>;

// using TreeWithStrangeCompare =
//     flat_tree<int, std::identity, NonDefaultConstructibleCompare, std::vector<int>>;
...
//         flat_tree<MoveThrows, std::identity, std::less<>, std::array<MoveThrows, 1>>;
...
//     using Tree = flat_tree<Pair, std::identity, LessByFirst<Pair>, std::vector<Pair>>;

//     // Constructors are stable.
//     Tree cont({{0, 0}, {1, 0}, {0, 1}, {2, 0}, {0, 2}, {1, 1}});

//     auto AllOfSecondsAreZero = [&cont] {
//         return ranges::all_of(cont, [](const Pair& elem) { return elem.second == 0; });
//     };
...
//         EXPECT_THAT(first_of, ElementsAre(IntPair(1, 1), IntPair(2, 1), IntPair(3, 1)));
//     }
//     {
//         TreeWithStrangeCompare::value_type input_vals[] = {1, 1, 1, 2, 2, 2, 3, 3, 3};
...
//     }
//     {
//         TreeWithStrangeCompare cont({1, 2, 3, 4, 5, 6, 10, 8}, NonDefaultConstructibleCompare(0));
//         EXPECT_THAT(cont, ElementsAre(1, 2, 3, 4, 5, 6, 8, 10));
//     }
//     {
...
//         IntPairTree first_of(sorted_unique, MakeInputIterator(std::begin(input_vals)),
//                              MakeInputIterator(std::end(input_vals)));
//         EXPECT_THAT(first_of, ElementsAre(IntPair(1, 1), IntPair(2, 1), IntPair(3, 1)));
//     }
//     {
...
//         TreeWithStrangeCompare cont(sorted_unique, MakeInputIterator(std::begin(input_vals)),
//                                     MakeInputIterator(std::end(input_vals)),
//                                     NonDefaultConstructibleCompare(0));
//         EXPECT_THAT(cont, ElementsAre(1, 2, 3));
//     }
...
//     auto size = static_cast<typename TypedTree<TypeParam>::difference_type>(cont.size());
...
//         EXPECT_EQ(rit, c_rit);
//         for (int j = static_cast<int>(size); rit != cont.rend(); ++rit, ++c_rit, --j) {
//             EXPECT_EQ(j, *rit);
//             EXPECT_EQ(j, *c_rit);
//         }
//     }
...
//     int value = 2;
//     std::pair<typename TypedTree<TypeParam>::iterator, bool> result = cont.insert(value);
...
//     using IntIntMap = flat_tree<int, GetKeyFromIntIntPair, std::less<int>, std::vector<IntPair>>;

//     {
//         IntIntMap cont;
//         IntPair int_pairs[] = {{3, 1}, {1, 1}, {4, 1}, {2, 1}};
//         UNSAFE_BUFFERS(cont.insert(std::begin(int_pairs), std::end(int_pairs)));
//         EXPECT_THAT(cont, ElementsAre(IntPair(1, 1), IntPair(2, 1), IntPair(3, 1), IntPair(4, 1)));
//     }
...
//         UNSAFE_BUFFERS(cont.insert(std::begin(int_pairs), std::end(int_pairs)));
//         EXPECT_THAT(cont, ElementsAre(IntPair(1, 1), IntPair(2, 1), IntPair(3, 1), IntPair(4, 1),
//                                       IntPair(5, 2), IntPair(6, 2), IntPair(7, 2), IntPair(8, 2)));
//     }
// }
...
//         std::pair<typename TypedTree<TypeParam>::iterator, bool> result = cont.emplace(2);
//         EXPECT_TRUE(result.second);
//         EXPECT_EQ(cont.begin(), result.first);
//         EXPECT_EQ(1U, cont.size());
//         EXPECT_EQ(2, *result.first);
...
//     auto it = cont.erase(std::next(cont.cbegin(), 5), std::next(cont.cbegin(), 5));
...
//         std::pair<typename TypedTree<TypeParam>::iterator, typename TypedTree<TypeParam>::iterator>
...
//     int unsorted[] = {2, 1};
//     EXPECT_DCHECK_DEATH(
//         TypedTree<TypeParam>(sorted_unique, std::begin(unsorted), std::end(unsorted)));

//     int repeated[] = {1, 2, 2};
//     EXPECT_DCHECK_DEATH(
//         TypedTree<TypeParam>(sorted_unique, std::begin(repeated), std::end(repeated)));
// }
...
// using IntSequenceContainers = ::testing::Types<std::deque<int>, std::vector<int>>;
