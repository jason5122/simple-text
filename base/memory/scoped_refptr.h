...
constexpr bool IsRefCountPreferenceOverridden(const T*, const RefCountedDeleteOnSequence<U>*) {
    return !std::same_as<std::decay_t<decltype(GetRefCountPreference<T>())>,
                         std::decay_t<decltype(GetRefCountPreference<U>())>>;
}
...
template <typename T, typename U, typename V>
constexpr void AssertRefCountBaseMatches(const T*, const RefCountedThreadSafe<U, V>*) {
    static_assert(std::derived_from<T, U>,
                  "T implements RefCountedThreadSafe<U>, but U is not a base of T.");
}

template <typename T, typename U>
constexpr void AssertRefCountBaseMatches(const T*, const RefCountedDeleteOnSequence<U>*) {
    static_assert(std::derived_from<T, U>,
...
public:
    ...
    ~scoped_refptr() {
        static_assert(!base::subtle::IsRefCountPreferenceOverridden(static_cast<T*>(nullptr),
    ...
    template <typename U>
    friend bool operator==(const scoped_refptr<T>& lhs, const scoped_refptr<U>& rhs) {
    ...
    template <typename U>
    friend auto operator<=>(const scoped_refptr<T>& lhs, const scoped_refptr<U>& rhs) {
        return lhs.ptr_ <=> rhs.ptr_;
    }
