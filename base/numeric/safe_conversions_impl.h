...
inline constexpr int kMaxExponent =
    std::is_floating_point_v<NumericType> ? std::numeric_limits<NumericType>::max_exponent
...
template <typename Dst,
          typename Src,
          IntegerRepresentation DstSign = std::is_signed_v<Dst> ? IntegerRepresentation::kSigned
                                                                : IntegerRepresentation::kUnsigned,
          IntegerRepresentation SrcSign = std::is_signed_v<Src> ? IntegerRepresentation::kSigned
                                                                : IntegerRepresentation::kUnsigned>
inline constexpr auto kStaticDstRangeRelationToSrcRange =
    kMaxExponent<Dst> >= kMaxExponent<Src> ? NumericRangeRepresentation::kContained
                                           : NumericRangeRepresentation::kNotContained;

// Unsigned to signed: Dst is guaranteed to contain source only if its range is
// larger.
template <typename Dst, typename Src>
inline constexpr auto kStaticDstRangeRelationToSrcRange<Dst,
                                                        Src,
...
struct NarrowingRange {
    ...
    static constexpr int kShift =
        (kMaxExponent<Src> > kMaxExponent<Dst> && SrcLimits::digits < DstLimits::digits)
            ? (DstLimits::digits - SrcLimits::digits)
            : 0;

    template <typename T>
        requires(std::same_as<T, Dst> && ((std::integral<T> && kShift < DstLimits::digits) ||
...
template <typename Dst,
          ...
          IntegerRepresentation SrcSign = std::is_signed_v<Src> ? IntegerRepresentation::kSigned
                                                                : IntegerRepresentation::kUnsigned,
          NumericRangeRepresentation DstRange = kStaticDstRangeRelationToSrcRange<Dst, Src>>
...
inline constexpr bool kIsTypeInRangeForNumericType =
    kStaticDstRangeRelationToSrcRange<Dst, Src> == NumericRangeRepresentation::kContained;
...
#define INTEGER_FOR_DIGITS_AND_SIGN(I)                                                            \
    template <>                                                                                   \
    struct IntegerForDigitsAndSignImpl<kIntegerBitsPlusSign<I>, std::is_signed_v<I>> {            \
        using type = I;                                                                           \
    }
...
template <size_t Size, bool IsSigned>
using IntegerForDigitsAndSign = IntegerForDigitsAndSignImpl<Size, IsSigned>::type;
...
using TwiceWiderInteger = IntegerForDigitsAndSign<kIntegerBitsPlusSign<Integer> * 2, IsSigned>;

// Determines the type that can represent the largest positive value.
template <typename Lhs, typename Rhs>
using MaxExponentPromotion = std::conditional_t<(kMaxExponent<Lhs> > kMaxExponent<Rhs>), Lhs, Rhs>;

// Determines the type that can represent the lowest arithmetic value.
template <typename Lhs, typename Rhs>
using LowestValuePromotion =
    std::conditional_t<std::is_signed_v<Lhs>
...
template <typename Lhs,
          typename Rhs = Lhs,
          bool is_intmax_type = std::is_integral_v<MaxExponentPromotion<Lhs, Rhs>> &&
                                kIntegerBitsPlusSign<MaxExponentPromotion<Lhs, Rhs>> ==
                                    kIntegerBitsPlusSign<intmax_t>,
          bool is_max_exponent =
              kStaticDstRangeRelationToSrcRange<MaxExponentPromotion<Lhs, Rhs>, Lhs> ==
                  NumericRangeRepresentation::kContained &&
              kStaticDstRangeRelationToSrcRange<MaxExponentPromotion<Lhs, Rhs>, Rhs> ==
                  NumericRangeRepresentation::kContained>
...
struct BigEnoughPromotionImpl<Lhs, Rhs, false, false> {
    using type = TwiceWiderInteger<MaxExponentPromotion<Lhs, Rhs>,
...
inline constexpr bool kIsIntegerArithmeticSafe =
    ...
    !std::is_floating_point_v<Rhs> && std::is_signed_v<T> >= std::is_signed_v<Lhs> &&
...
template <typename Lhs, typename Rhs>
    requires(
        kIsIntegerArithmeticSafe<std::conditional_t<std::is_signed_v<Lhs> || std::is_signed_v<Rhs>,
                                                    intmax_t,
                                                    uintmax_t>,
...
template <typename Lhs, typename Rhs>
using FastIntegerArithmeticPromotion = FastIntegerArithmeticPromotionImpl<Lhs, Rhs>::type;
...
template <typename T>
using ArithmeticOrUnderlyingEnum = typename std::
...
template <typename Src,
          typename Dst = std::make_signed_t<typename base::internal::UnderlyingType<Src>::type>>
...
template <typename Src,
          typename Dst = std::make_unsigned_t<typename base::internal::UnderlyingType<Src>::type>>
    requires std::integral<Dst>
...
struct IsLess {
    ...
    static constexpr bool Test(L lhs, R rhs) {
        ...
        return l_range.IsUnderflow() || r_range.IsOverflow() ||
               (l_range == r_range && static_cast<SumT>(lhs) < static_cast<SumT>(rhs));
    }
};
...
constexpr bool SafeCompare(L lhs, R rhs) {
    ...
    return kIsBigEnoughPromotionContained<L, R>
               ...
               ? C<BigType, BigType>::Test(static_cast<BigType>(lhs), static_cast<BigType>(rhs))
               // Let the template functions figure it out for mixed types.
               : C<L, R>::Test(lhs, rhs);
}

template <typename Dst, typename Src>
inline constexpr bool kIsMaxInRangeForNumericType = IsGreaterOrEqual<Dst, Src>::Test(
    std::numeric_limits<Dst>::max(), std::numeric_limits<Src>::max());

template <typename Dst, typename Src>
inline constexpr bool kIsMinInRangeForNumericType = IsLessOrEqual<Dst, Src>::Test(
    std::numeric_limits<Dst>::lowest(), std::numeric_limits<Src>::lowest());

template <typename Dst, typename Src>
inline constexpr Dst kCommonMax =
    kIsMaxInRangeForNumericType<Dst, Src> ? static_cast<Dst>(std::numeric_limits<Src>::max())
                                          : std::numeric_limits<Dst>::max();

template <typename Dst, typename Src>
inline constexpr Dst kCommonMin =
    kIsMinInRangeForNumericType<Dst, Src> ? static_cast<Dst>(std::numeric_limits<Src>::lowest())
