...
#include "experiments/message_loop/time/time.h"
#include <atomic>
#include <cmath>
#include <limits>
#include <optional>
...
std::atomic<TimeNowFunction> g_time_now_function{&subtle::TimeNowIgnoringOverride};

std::atomic<TimeNowFunction> g_time_now_from_system_time_function{
    &subtle::TimeNowFromSystemTimeIgnoringOverride};

std::atomic<TimeTicksNowFunction> g_time_ticks_now_function{&subtle::TimeTicksNowIgnoringOverride};

std::atomic<TimeTicksLowResolutionNowFunction> g_time_ticks_low_resolution_now_function{
    &subtle::TimeTicksLowResolutionNowIgnoringOverride};

std::atomic<LiveTicksNowFunction> g_live_ticks_now_function{&subtle::LiveTicksNowIgnoringOverride};
...
TimeDelta TimeDelta::CeilToMultiple(TimeDelta interval) const {
    ...
    return remainder.is_zero() ? *this : (*this - remainder + interval.magnitude());
}
...
// static
Time Time::Now() { return internal::g_time_now_function.load(std::memory_order_relaxed)(); }

// static
Time Time::NowFromSystemTime() {
    // Just use g_time_now_function because it returns the system time.
    return internal::g_time_now_from_system_time_function.load(std::memory_order_relaxed)();
}
...
Time Time::Midnight(bool is_local) const {
    ...
    Time out_time;
    if (FromExploded(is_local, exploded, &out_time)) return out_time;
...
bool Time::ExplodedMostlyEquals(const Exploded& lhs, const Exploded& rhs) {
    return std::tie(lhs.year, lhs.month, lhs.day_of_month, lhs.hour, lhs.minute, lhs.second,
                    lhs.millisecond) == std::tie(rhs.year, rhs.month, rhs.day_of_month, rhs.hour,
                                                 rhs.minute, rhs.second, rhs.millisecond);
}

// static
bool Time::FromMillisecondsSinceUnixEpoch(int64_t unix_milliseconds, Time* time) {
...
TimeTicks TimeTicks::LowResolutionNow() {
    return internal::g_time_ticks_low_resolution_now_function.load(std::memory_order_relaxed)();
}
...
TimeTicks TimeTicks::UnixEpoch() {
    struct StaticUnixEpoch {
        StaticUnixEpoch()
            : epoch(g_shared_time_ticks_at_unix_epoch.is_null()
...
TimeTicks TimeTicks::SnappedToNextTick(TimeTicks tick_phase, TimeDelta tick_interval) const {
    ...
    // tick after |this|.
    if (!interval_offset.is_zero() && tick_phase < *this) interval_offset += tick_interval;
...
ThreadTicks ThreadTicks::Now() {
    return internal::g_thread_ticks_now_function.load(std::memory_order_relaxed)();
}
