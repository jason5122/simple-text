...
constexpr bool isnan(double d) { return d != d; }
...
public:
    ...
    constexpr TimeDelta& operator+=(TimeDelta other) { return *this = (*this + other); }
    constexpr TimeDelta& operator-=(TimeDelta other) { return *this = (*this - other); }
    ...
    constexpr int64_t IntDiv(TimeDelta a) const {
        ...
        CHECK(!is_zero() || !a.is_zero());
        CHECK(!is_inf() || !a.is_inf());
        return ((delta_ < 0) == (a.delta_ < 0)) ? std::numeric_limits<int64_t>::max()
                                                : std::numeric_limits<int64_t>::min();
    }

    constexpr TimeDelta operator%(TimeDelta a) const {
        return TimeDelta((is_inf() || a.is_zero() || a.is_inf()) ? delta_ : (delta_ % a.delta_));
    }
    constexpr TimeDelta& operator%=(TimeDelta other) { return *this = (*this % other); }

    // Comparison operators.
    friend constexpr bool operator==(TimeDelta, TimeDelta) = default;
    friend constexpr std::strong_ordering operator<=>(TimeDelta, TimeDelta) = default;
...
public:
    ...
    static constexpr int64_t kMicrosecondsPerMinute = kMicrosecondsPerSecond * kSecondsPerMinute;
    static constexpr int64_t kMicrosecondsPerHour = kMicrosecondsPerMinute * kMinutesPerHour;
    ...
    static constexpr TimeClass Max() { return TimeClass(std::numeric_limits<int64_t>::max()); }

    static constexpr TimeClass Min() { return TimeClass(std::numeric_limits<int64_t>::min()); }
    ...
    friend constexpr std::strong_ordering operator<=>(const TimeBase&, const TimeBase&) = default;
...
// UTCExplode() and LocalExplode(). See method comments for further details.
    ...
    constexpr TimeDelta ToDeltaSinceWindowsEpoch() const { return Microseconds(us_); }
    ...
    [[nodiscard]] static bool FromUTCExploded(const Exploded& exploded, Time* time) {
        return FromExploded(false, exploded, time);
    }
    [[nodiscard]] static bool FromLocalExploded(const Exploded& exploded, Time* time) {
    ...
    [[nodiscard]] static bool FromString(const char* time_string, Time* parsed_time) {
        return FromStringInternal(time_string, true, parsed_time);
    }
    [[nodiscard]] static bool FromUTCString(const char* time_string, Time* parsed_time) {
...
private:
    ...
    [[nodiscard]] static bool FromExploded(bool is_local, const Exploded& exploded, Time* time);
    ...
    static void ExplodeUsingIcu(int64_t millis_since_unix_epoch,
                                bool is_local,
                                Exploded* exploded);
    [[nodiscard]] static bool FromExplodedUsingIcu(bool is_local,
    ...
    [[nodiscard]] static bool ExplodedMostlyEquals(const Exploded& lhs, const Exploded& rhs);

    // Converts the provided time in milliseconds since the Unix epoch (1970) to a
    // Time object, avoiding overflows.
    [[nodiscard]] static bool FromMillisecondsSinceUnixEpoch(int64_t unix_milliseconds,
                                                             Time* time);
...
constexpr TimeDelta Days(T n) {
    return TimeDelta::FromInternalValue(MakeClampedNum(n) * Time::kMicrosecondsPerDay);
}
template <typename T>
constexpr TimeDelta Hours(T n) {
    return TimeDelta::FromInternalValue(MakeClampedNum(n) * Time::kMicrosecondsPerHour);
}
template <typename T>
constexpr TimeDelta Minutes(T n) {
    return TimeDelta::FromInternalValue(MakeClampedNum(n) * Time::kMicrosecondsPerMinute);
}
template <typename T>
constexpr TimeDelta Seconds(T n) {
    return TimeDelta::FromInternalValue(MakeClampedNum(n) * Time::kMicrosecondsPerSecond);
}
template <typename T>
constexpr TimeDelta Milliseconds(T n) {
    return TimeDelta::FromInternalValue(MakeClampedNum(n) * Time::kMicrosecondsPerMillisecond);
}
template <typename T>
constexpr TimeDelta Microseconds(T n) {
...
constexpr TimeDelta Nanoseconds(T n) {
    return TimeDelta::FromInternalValue(MakeClampedNum(n) / Time::kNanosecondsPerMicrosecond);
}
template <typename T>
constexpr TimeDelta Hertz(T n) {
    return n ? TimeDelta::FromInternalValue(Time::kMicrosecondsPerSecond / MakeClampedNum(n))
             : TimeDelta::Max();
}
...
constexpr int TimeDelta::InDays() const {
    ...
    return (delta_ < 0) ? std::numeric_limits<int>::min() : std::numeric_limits<int>::max();
}
...
constexpr int TimeDelta::InDaysFloored() const {
    if (!is_inf()) {
        ...
        return (result * Time::kMicrosecondsPerDay > delta_) ? (result - 1) : result;
    }
    return (delta_ < 0) ? std::numeric_limits<int>::min() : std::numeric_limits<int>::max();
}
...
constexpr double TimeDelta::InSecondsF() const {
    if (!is_inf()) return static_cast<double>(delta_) / Time::kMicrosecondsPerSecond;
    return (delta_ < 0) ? -std::numeric_limits<double>::infinity()
                        : std::numeric_limits<double>::infinity();
}
...
constexpr int64_t TimeDelta::InSecondsFloored() const {
    if (!is_inf()) {
        ...
        // Convert |result| from truncating to flooring.
        return (result * Time::kMicrosecondsPerSecond > delta_) ? (result - 1) : result;
    }
    return delta_;
}
...
constexpr int64_t TimeDelta::InMillisecondsRoundedUp() const {
    if (!is_inf()) {
        ...
        return (delta_ > result * Time::kMicrosecondsPerMillisecond) ? (result + 1) : result;
    }
    return delta_;
}
...
// static
constexpr TimeDelta TimeDelta::Max() { return TimeDelta(std::numeric_limits<int64_t>::max()); }

// static
constexpr TimeDelta TimeDelta::Min() { return TimeDelta(std::numeric_limits<int64_t>::min()); }
...
template <class TimeClass>
constexpr TimeDelta TimeBase<TimeClass>::operator-(const TimeBase<TimeClass>& other) const {
...
constexpr Time Time::FromTimeT(time_t tt) {
    ...
    return (tt == std::numeric_limits<time_t>::max()) ? Max() : (UnixEpoch() + Seconds(tt));
}
...
constexpr time_t Time::ToTimeT() const {
    ...
    return (us_ < 0) ? std::numeric_limits<time_t>::min() : std::numeric_limits<time_t>::max();
}
...
constexpr Time Time::FromTimeSpec(const timespec& ts) {
    return FromSecondsSinceUnixEpoch(ts.tv_sec +
...
constexpr int64_t Time::InMillisecondsSinceUnixEpoch() const {
    ...
    return (us_ < 0) ? std::numeric_limits<int64_t>::min() : std::numeric_limits<int64_t>::max();
}
...
class LiveTicks : public time_internal::TimeBase<LiveTicks> {
public:
...
public:
...
#if (defined(_POSIX_THREAD_CPUTIME) && (_POSIX_THREAD_CPUTIME >= 0)) || BUILDFLAG(IS_APPLE) ||    \
    ...
    static constexpr ThreadTicks FromInternalValue(int64_t us) { return ThreadTicks(us); }
