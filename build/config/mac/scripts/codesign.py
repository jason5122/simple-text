...
def FindProvisioningProfile(provisioning_profile_paths, bundle_identifier, required):
...
def GenerateEntitlements(path, provisioning_profile, bundle_identifier):
    ...
    if provisioning_profile:
        ...
        app_identifier_prefix = provisioning_profile.application_identifier_prefix + '.'
    else:
    ...
    def _Execute(args):
        ...
        if args.embedded_mobileprovision:
            ...
            provisioning_profile = FindProvisioningProfile(
                args.mobileprovision_files, bundle.identifier, provisioning_profile_required
            )
            if provisioning_profile and args.platform != 'iphonesimulator':
                ...
                if args.entitlements_path is not None:
                    ...
                    codesign_extra_args.extend(['--entitlements', temporary_entitlements_file.name])
...
class CodeSignFileAction(Action):
    ...
    def _Register(parser):
        ...
        parser.add_argument('--identity', '-i', required=True, help='identity to use to codesign')
        parser.add_argument(
            '--output', '-o', help='if specified copy the file to that location before signing it'
        )
    ...
    def _Execute(args):
        ...
        CodeSignBundle(
            install_path, args.identity, ['--deep', '--preserve-metadata=identifier,entitlements']
        )
...
class GenerateEntitlementsAction(Action):
    ...
    def _Register(parser):
        ...
        parser.add_argument('path', help='path to the entitlements file to generate')
        parser.add_argument(
            '--info-plist', '-p', required=True, help='path to the bundle Info.plist'
        )
        parser.add_argument(
            '--mobileprovision',
            '-m',
...
class FindProvisioningProfileAction(Action):
    ...
    @staticmethod
    def _Register(parser):
        parser.add_argument('--bundle-id', '-b', required=True, help='bundle identifier')
        parser.add_argument(
            '--mobileprovision',
            '-m',
            action='append',
