...
public:
    ...
    bool initialize(std::string* error) {
        ...
        g_variant_builder_add(&stream_properties, "{sv}", "cursor-mode", g_variant_new_uint32(0));
        ...
        signal_subscription_ = g_dbus_connection_signal_subscribe(
            ...
            nullptr);
        reply =
            call(connection_, session_path_.c_str(), kSessionInterface, "Start", nullptr, &glib);
        if (!reply) {
...
private:
    double primary_monitor_scale() {
        ...
        GVariant* reply = g_dbus_connection_call_sync(
            connection_, kDisplayConfigDestination, kDisplayConfigPath, kDisplayConfigInterface,
            "GetCurrentState", nullptr, nullptr, G_DBUS_CALL_FLAGS_NONE, -1, nullptr, &error);
        if (!reply) {
