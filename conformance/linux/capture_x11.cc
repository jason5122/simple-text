...
unsigned long window_pid(Display* display, Window window) {
    ...
    const int status =
        XGetWindowProperty(display, window, pid_atom, 0, 1, False, XA_CARDINAL, &actual_type,
                           &actual_format, &count, &remaining, &property);
    unsigned long pid = 0;
    ...
    bool next_frame(Frame* frame, Crop crop, std::chrono::milliseconds timeout) override {
        ...
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                auto* destination = frame->rgba.data() + (static_cast<size_t>(y) * width + x) * 4;
                destination[0] = channel(pixel, red_mask);
                destination[1] = channel(pixel, green_mask);
                destination[2] = channel(pixel, blue_mask);
                destination[3] =
                    attributes.depth == 32 && alpha_mask ? channel(pixel, alpha_mask) : 255;
            }
        }
        XDestroyImage(image);
        return true;
