...
TEST(PieceTreeTest, Assignment) {
    using namespace std::string_literals;
    using namespace std::string_view_literals;
    PieceTree tree;
    tree = "one";
    EXPECT_EQ(tree.str(), "one");
    tree = "two"s;
    EXPECT_EQ(tree.str(), "two");
    tree = "three"sv;
    EXPECT_EQ(tree.str(), "three");
}
...
TEST(PieceTreeTest, InsertAtRandom) {
    ...
    for (size_t n = 0; n < 100; ++n) {
        ...
        const std::string random_str = base::rand_bytes_as_string(base::rand_int(0, 10));
...
TEST(PieceTreeTest, RandomTestLite) {
    ...
    for (size_t n = 0; n < 5; ++n) {
        ...
        // Randomly erase.
        size_t erase_index = base::rand_int(0, str.length());
        size_t count = base::rand_int(0, 4);
...
TEST(PieceTreeTest, EraseAtRandom) {
    ...
    for (size_t n = 0; n < 100; ++n) {
        ...
        for (size_t i = 0; i < 10; ++i) {
            size_t index = base::rand_int(0, str.length());
            size_t count = base::rand_int(0, str.length());
...
TEST(PieceTreeTest, LineColumnAt3) {
    ...
    EXPECT_EQ(cursor1.line, 0UZ);
    EXPECT_EQ(cursor1.column, 2UZ);

    auto cursor2 = tree.line_column_at(7);
    EXPECT_EQ(cursor2.line, 1UZ);
    EXPECT_EQ(cursor2.column, 1UZ);
}
...
TEST(PieceTreeTest, LineColumnAtRandomTest) {
    ...
    for (size_t n = 0; n < 50; ++n) {
        ...
        // Randomly erase.
        size_t erase_index = base::rand_int(0, str.length());
        size_t erase_count = base::rand_int(0, 4);
...
TEST(PieceTreeTest, GetLineContentAfterInsertTest1) {
    ...
    EXPECT_EQ(tree.line_feed_count(), 2UZ);
    EXPECT_EQ(tree.line_count(), 3UZ);
}
...
TEST(PieceTreeTest, FindTest1) {
    ...
    EXPECT_EQ(*tree.find("world"), 6UZ);

    EXPECT_FALSE(tree.find("z"));

    ASSERT_TRUE(tree.find("hello world"));
    EXPECT_EQ(*tree.find("hello world"), 0UZ);
}
...
TEST(PieceTreeTest, FindTest2) {
    ...
    EXPECT_EQ(*tree.find("😀"), 6UZ);

    EXPECT_FALSE(tree.find("Ò"));

    ASSERT_TRUE(tree.find("🙂↕️"));
    EXPECT_EQ(*tree.find("🙂↕️"), 10UZ);

    ASSERT_TRUE(tree.find("\x8F"));
    EXPECT_EQ(*tree.find("\x8F"), 5UZ);
