...
std::u32string make_line(std::u32string_view prefix, std::initializer_list<char32_t> codepoints) {
...
public:
    ControlCharDemo()
        : heading_font_(px_create_font("system", 20.0f, PX_FONT_BOLD)),
          ...
          name_shaper_(std::make_unique<grapheme_shaper>(
              body_font_,
              GRAPHEME_SHAPER_DRAW_COMMON_WHITESPACE | GRAPHEME_SHAPER_DRAW_BIDI |
    ...
    void paint(px_render_context* context, rect bounds, const rect*, int) override {
        ...
        context->draw_text(
            body_font_, {48.0, 292.0}, kTextColor,
            "Mnemonic policy, including optional whitespace and bidi markers:", true);
...
int main(int argc, char** argv) {
    ...
    px_window_t* window =
        px_create_window(&demo, nullptr, kWindowWidth, kWindowHeight, "control character demo",
