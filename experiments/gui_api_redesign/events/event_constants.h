...
constexpr EventFlags EF_MOUSE_BUTTON = EF_LEFT_MOUSE_BUTTON | EF_MIDDLE_MOUSE_BUTTON |
...
enum EventResult {
    ...
    ER_CONSUMED = 1 << 1,               // The event has been handled, and it should not be
                                        // propagated to other handlers.
    ER_DISABLE_SYNC_HANDLING = 1 << 2,  // The event shouldn't be handled synchronously. This
                                        // happens if the event is being handled
                                        // asynchronously, or if the event is invalid and
                                        // shouldn't be handled at all.
    ER_FORCE_PROCESS_GESTURE = 1 << 3,  // The event should be processed by gesture recognizer even
                                        // if ER_HANDLED or ER_CONSUMED is set.
    ER_SKIPPED = 1 << 4,                // The event has been been handled, and it should not be
...
enum EventPhase { EP_PREDISPATCH, EP_PRETARGET, EP_TARGET, EP_POSTTARGET, EP_POSTDISPATCH };
