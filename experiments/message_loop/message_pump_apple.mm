...
public:
    ScopedModeEnabler(MessagePumpCFRunLoopBase* owner, int mode_index)
        ...
        CFRunLoopAddSource(loop, owner_->nesting_deferred_work_source_.get(), mode());
    ...
    ~ScopedModeEnabler() {
        ...
        CFRunLoopRemoveSource(loop, owner_->nesting_deferred_work_source_.get(), mode());
    ...
    const CFStringRef& mode() const {
        ...
        static_assert((1 << kNumModes) - 1 == kAllModesMask, "kAllModesMask not large enough");
...
void MessagePumpCFRunLoopBase::OnDidQuit() { keep_running_ = false; }
...
// Must be called on the run loop thread.
void MessagePumpCFRunLoopBase::ScheduleDelayedWork(const Delegate::NextWorkInfo& next_work_info) {
...
TimeTicks MessagePumpCFRunLoopBase::AdjustDelayedRunTime(TimeTicks earliest_time,
                                                         TimeTicks run_time,
                                                         TimeTicks latest_time) {
    if (g_timer_slack.load(std::memory_order_relaxed)) {
        return earliest_time;
    }
    return MessagePump::AdjustDelayedRunTime(earliest_time, run_time, latest_time);
}
...
MessagePumpCFRunLoopBase::MessagePumpCFRunLoopBase(int initial_mode_mask) {
    ...
    nesting_deferred_work_source_.reset(CFRunLoopSourceCreate(/*allocator=*/nullptr,
                                                              /*order=*/0,
                                                              /*context=*/&source_context));

    CFRunLoopObserverContext observer_context = {0};
    observer_context.info = this;
    pre_wait_observer_.reset(CFRunLoopObserverCreate(/*allocator=*/nullptr,
    ...
    after_wait_observer_.reset(CFRunLoopObserverCreate(
        ...
        /*context=*/&observer_context));
    pre_source_observer_.reset(CFRunLoopObserverCreate(/*allocator=*/nullptr,
...
MessagePumpCFRunLoopBase::~MessagePumpCFRunLoopBase() { SetModeMask(0); }

// static
void MessagePumpCFRunLoopBase::InitializeFeatures() {
    g_timer_slack.store(FeatureList::IsEnabled(kTimerSlackMac), std::memory_order_relaxed);
}
...
bool MessagePumpCFRunLoopBase::ShouldCreateAutoreleasePool() { return true; }
...
void MessagePumpCFRunLoopBase::SetModeMask(int mode_mask) {
    for (size_t i = 0; i < kNumModes; ++i) {
        ...
        if (enable == !enabled_modes_[i]) {
            enabled_modes_[i] = enable ? std::make_unique<ScopedModeEnabler>(this, i) : nullptr;
        }
    }
}
...
MessagePumpNSRunLoop::MessagePumpNSRunLoop() : MessagePumpCFRunLoopBase(kCommonModeMask) {
    CFRunLoopSourceContext source_context = {0};
...
#if BUILDFLAG(IS_IOS)
MessagePumpUIApplication::MessagePumpUIApplication() : MessagePumpCFRunLoopBase(kCommonModeMask) {}
...
bool MessagePumpUIApplication::DoQuit() { NOTREACHED(); }
...
void MessagePumpNSApplication::EnterExitRunLoop(CFRunLoopActivity activity) {
    ...
    if (activity == kCFRunLoopEntry && quit_pending_ && nesting_level() <= run_nesting_level() &&
