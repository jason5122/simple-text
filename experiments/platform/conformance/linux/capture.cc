...
Frame capture_frame(WindowId window_id, Crop crop) {
    ...
    if (use_software) {
        ...
        for (int y = 0; y < height; ++y) {
            const std::uint8_t* source_row =
                source + static_cast<size_t>(top + y) * source_row_bytes +
    ...
    glFinish();
    glReadPixels(left, buffer_height - top - height, width, height, GL_RGBA,
...
Frame wait_settled(WindowId window_id, Crop crop, Frame baseline) {
    ...
    while (std::chrono::duration<double>(std::chrono::steady_clock::now() - started_at).count() <
        ...
        const bool changed =
            !baseline || !frames_equal(static_cast<LinuxFrame*>(current),
...
WindowId find_window(const char*) {
