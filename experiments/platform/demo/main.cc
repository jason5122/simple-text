...
constexpr const char* kDemoTitle = "platform - gl_render_context demo";
...
const char* event_name(px_event_type type) {
    ...
    case PX_EVENT_FOCUS_LOST:
        return "focus_lost";
    case PX_EVENT_SETTINGS_CHANGED:
...
public:
    ...
    bool handle_event(const px_event_t* event) override {
        ...
        case PX_EVENT_SCROLL:
            ...
            // px's positive scroll delta means that content moves up. Keep this as a logical
            // viewport offset and apply the sign once while drawing; retaining the fractional
            // value is useful for judging trackpad cadence and sub-pixel motion.
            scroll_offset_ =
        ...
        case PX_EVENT_DROP_FILES:
            std::println("[{:2} {:<16}] {} path(s)", tag, event_name(event->type),
                         event->path_count);
    ...
    void draw(px_render_context* rc, rect bounds, const rect* dirty, int dirty_count) override {
        ...
        // contrast edges make uneven frame cadence much easier to see than the old single bar.
        // Cull rows outside the viewport as a real editor would, while leaving a little overscan
        // at either edge.
        ...
        for (int row = 0; row < kRowCount; ++row) {
            ...
            rc->draw_rect(rect{88.0, y + 10.0, title_width, 5.0},
                          section ? fcolor{0.70f, 0.84f, 0.96f, 1.0f}
                                  : fcolor{0.63f, 0.66f, 0.72f, 1.0f});
            rc->draw_rect(rect{88.0, y + 25.0, detail_width, 3.0},
                          fcolor{0.37f, 0.41f, 0.48f, 1.0f});
            rc->draw_rect(rect{88.0, y + 36.0, document_width - 72.0, 1.0},
        ...
        // A bar driven by animation_tick, to show the display link is live. Its vertical position
        // is fixed to the viewport so it remains separate from the static scroll test above.
        ...
        if (hover_ && hover_->inside()) {
            ...
            // Keep terminal I/O out of the render path by default. This old event-to-draw probe
            // excludes CA presentation time; PX_LAG_TRACE enables the native event-to-target trace
            // in the layer.
    ...
    // ── px_input_client
    // ─────────────────────────────────────────────────────────────────────────── Enough of
    // NSTextInputClient to prove the IME path is wired. No text rendering, so committed text is
...
public:
    ...
    void animation_tick(double now) override {
        ...
        // remaining source of redraws is discrete input events (mouse move, resize, scroll). A/B
        // test for whether continuous animation-driven redraw is what pushes
        // CAOpenGLLayer/WindowServer into a steady-state pipelined mode with several frames in
        // flight -- fixed extra latency that wouldn't show up as growing staleness in the input
        // data itself, only as everything on screen being consistently a few frames behind.
...
int main(int argc, char** argv) {
    // The event log is the useful output of this demo, and it is meant to be diffed against a
    // trace from another implementation. Block buffering would lose it on a kill and reorder it
    // against stderr, so opt out.
    //
