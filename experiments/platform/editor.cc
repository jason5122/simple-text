...
double layout_advance(const LayoutBatches& batches) {
    if (batches.empty()) {
        return 0.0;
    }
    const fx_layout_batch& last = batches.back();
    return last.x_offset + static_cast<double>(last.layout.advance);
}

HighlightedLine highlight_line(px_font_t* font, std::string_view text) {
...
void draw_layout(px_render_context* context,
                 px_font_t* font,
                 vec2 position,
                 fcolor color,
                 LayoutBatches* batches) {
