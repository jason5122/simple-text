...
fx_glyph_bitmap convert_bitmap(fx_detail::GlyphBitmap bitmap) {
    return {
        .width = bitmap.width,
...
public:
    ...
    std::unique_ptr<fx_layout> shape(std::string_view utf8) override {
        const fx_detail::ShapedText shaped = fx_detail::shape(handle_, utf8);
        auto layout = std::make_unique<fx_layout>();
        layout->advance = shaped.advance;
        layout->line_height = shaped.line_height;
        layout->primary_y_offset = shaped.primary_y_offset;
        layout->primary_face_ascent = shaped.primary_face_ascent;
        layout->glyphs.reserve(shaped.glyphs.size());
        for (const fx_detail::GlyphPlacement& glyph : shaped.glyphs) {
            layout->glyphs.push_back({
                .id = glyph.glyph_id,
                .advance = glyph.x_advance,
                .x_offset = glyph.x_offset,
                .y_offset = glyph.y_offset,
                .face_ascent = glyph.face_ascent,
    ...
    fx_glyph_bitmap rasterise(uint32_t glyph, float scale, float subpixel_x) override {
        return convert_bitmap(fx_detail::rasterize(handle_, glyph, scale, subpixel_x));
    }
...
std::unique_ptr<fx_font> fx_create_font(std::string_view family, float size, uint32_t attrs) {
    std::optional<fx_detail::FontHandle> handle = fx_detail::create_font(
        ...
        attrs & FX_FONT_BOLD ? fx_detail::Weight::Bold : fx_detail::Weight::Normal,
        attrs & FX_FONT_ITALIC ? fx_detail::Slant::Italic : fx_detail::Slant::Normal, attrs);
    if (!handle) {
        return nullptr;
    }
