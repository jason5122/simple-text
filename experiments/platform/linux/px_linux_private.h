...
// version -- not part of the platform abstraction, so it is not reproduced here: this backend links
// gtk+-3.0 normally through pkg-config, as instructed. The GTK3 call sequence itself mirrors what
// the recovered symbol list says ST actually calls.
...
struct px_window_t {
  ...
  // backings on macOS and Windows there is no framebuffer handed to us. This one is owned, sized to
  // the drawing area's allocation in device pixels, and blitted into the "draw" signal's cairo_t
  // with gdk_cairo_draw_from_gl every frame. Recreated on resize.
  GLuint fbo = 0;
  GLuint color_renderbuffer = 0;
  int fbo_width = 0;
  int fbo_height = 0;

  // Currently-held keyvals, so key-press-event can report repeat. GDK's detectable-autorepeat mode
  // (the default) sends only repeated press events with no interleaved release, unlike NSEvent's
  // isARepeat or WM_KEYDOWN's lParam bit 30 -- there is no field to read it from directly.
  ...
  // exclusive WM_LBUTTONDOWN/WM_LBUTTONDBLCLK, so counting has to be done by hand from timing and
  // position the way ST's confirmed use of those two settings implies.
