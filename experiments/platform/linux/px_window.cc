...
// identified GTK3 equivalent -- GDK's implicit per-widget pointer grab during a button press has no
// "someone stole it" signal the way Win32's SetCapture/ReleaseCapture does -- so this backend never
// emits it.
...
GdkCursorType gdk_cursor_type_for(px_cursor_t cursor) {
  switch (cursor) {
    case PX_CURSOR_IBEAM: return GDK_XTERM;
    case PX_CURSOR_CROSSHAIR: return GDK_CROSSHAIR;
    case PX_CURSOR_POINTING_HAND: return GDK_HAND2;
    case PX_CURSOR_RESIZE_LEFT_RIGHT: return GDK_SB_H_DOUBLE_ARROW;
    case PX_CURSOR_RESIZE_UP_DOWN: return GDK_SB_V_DOUBLE_ARROW;
    case PX_CURSOR_ARROW: default: return GDK_LEFT_PTR;
  }
}
...
gboolean on_draw(GtkWidget* widget, cairo_t* cr, gpointer data) {
  ...
  if (window->use_gl && px_linux_gl_make_current(window) &&
    ...
    GLRenderContext rc(vec2{static_cast<double>(device_w), static_cast<double>(device_h)}, scale);
    window->handler->paint(&rc, bounds, dirty.data(), static_cast<int>(dirty.size()));

    // Single-buffered, like the other two backends: glFlush is the whole presentation step, and
    // gdk_cairo_draw_from_gl is what gets the result from our FBO into GTK3's cairo-composited
    // window -- GTK3 has no GL-native present path the way CAOpenGLLayer or an HDC-backed HGLRC do.
    glFlush();
...
void on_size_allocate(GtkWidget*, GtkAllocation* allocation, gpointer data) {
  ...
  const vec2 size{static_cast<double>(allocation->width), static_cast<double>(allocation->height)};
...
gboolean on_key_press_event(GtkWidget*, GdkEventKey* event, gpointer data) {
  ...
  // motivates macOS's "bindings first" comment (Cmd-S must not type 's'), this is safe here because
  // every real IM context (the plain "simple" engine, ibus, ...) already declines to filter
  // Ctrl/Alt-modified keys -- those are reserved for application shortcuts by convention -- so
  // chord bindings still reach handle_event below unfiltered.
  if (window->im_context && gtk_im_context_filter_keypress(window->im_context, event)) {
    return TRUE;
  }

  const bool is_repeat = std::find(window->pressed_keyvals.begin(), window->pressed_keyvals.end(),
                                   event->keyval) != window->pressed_keyvals.end();
  if (!is_repeat) {
...
px_mouse_button button_from_gdk(guint button) {
  switch (button) {
    case 1: return PX_MOUSE_LEFT;
    case 2: return PX_MOUSE_MIDDLE;
    case 3: return PX_MOUSE_RIGHT;
    case 8: return PX_MOUSE_X1;
    case 9: return PX_MOUSE_X2;
    default: return PX_MOUSE_NONE;
  }
}
...
gboolean on_button_press_event(GtkWidget* widget, GdkEventButton* event, gpointer data) {
  ...
  // GDK_2BUTTON_PRESS/GDK_3BUTTON_PRESS fire in addition to a plain GDK_BUTTON_PRESS for the same
  // physical click, not instead of it (unlike Win32's mutually exclusive WM_LBUTTONDOWN/DBLCLK) --
  // counting those too would double-report every multi-click. Click count is tracked by hand below
  // against gtk-double-click-time/-distance instead.
  ...
  const bool within_time = window->last_click_time != 0 &&
                           elapsed <= static_cast<guint32>(double_click_time);
  const bool within_distance = std::abs(pos.x - window->last_click_pos.x) <= double_click_distance &&
...
gboolean on_scroll_event(GtkWidget*, GdkEventScroll* event, gpointer data) {
  ...
  if (event->direction == GDK_SCROLL_SMOOTH) {
    ...
    // GdkEventScroll::delta_x/delta_y are plain struct fields (no accessor needed, which is why
    // neither appears in the resolved symbol list); gdk_event_is_scroll_stop_event is the one
    // confirmed call, used to detect a trackpad fling's terminal zero-delta event rather than to
    // read the deltas themselves. GDK's sign convention is "positive moves content right/down",
    // the opposite of px.h's "positive dy scrolls content up", so both axes are negated.
    e.scroll_delta = vec2{-event->delta_x * 16.0, -event->delta_y * 16.0};
  } else {
    e.precise_scroll = false;
    constexpr double kStep = 16.0 * 3.0;
    switch (event->direction) {
      case GDK_SCROLL_UP: e.scroll_delta = vec2{0.0, kStep}; break;
      case GDK_SCROLL_DOWN: e.scroll_delta = vec2{0.0, -kStep}; break;
      case GDK_SCROLL_LEFT: e.scroll_delta = vec2{kStep, 0.0}; break;
      case GDK_SCROLL_RIGHT: e.scroll_delta = vec2{-kStep, 0.0}; break;
      default: break;
    }
  }
  px_linux_send_event(window, &e);
...
gboolean on_drag_motion(GtkWidget*, GdkDragContext* context, gint x, gint y, guint time,
                        gpointer data) {
  px_window_t* window = static_cast<px_window_t*>(data);
  if (!window || !window->handler) {
    return FALSE;
  ...
  const bool accept = window->drag_active
                         ? window->handler->drag_drop_motion(pos, drop_paths().data(),
                                                             drop_paths().count())
                         : window->handler->drag_drop_enter(pos, drop_paths().data(),
                                                             drop_paths().count());
...
gboolean on_drag_drop(GtkWidget* widget, GdkDragContext* context, gint, gint, guint time,
                      gpointer) {
...
void px_linux_send_event(px_window_t* window, px_event_t* event) {
  ...
  // Key (0) and character (1) events skip the repaint flush; anything from mouse button (2) up can
  // trigger one, rate-limited -- the same tail every backend's send_event has.
...
void px_linux_flush_dirty_rects(px_window_t* window) {
  ...
  // GTK widget coordinates are already application pixels (== px.h's points at any scale factor),
  // unlike Win32's physical-pixel client area, so no scale multiply is needed here.
  for (const rect& r : window->dirty) {
    gtk_widget_queue_draw_area(window->area, static_cast<int>(std::floor(r.x)),
...
px_window_t* px_create_window(px_window_event_handler* handler,
  ...
  // double-buffering via GL surfaces), but it is the confirmed symbol ST resolves, so the warning
  // is suppressed the same way mac/px_gl_layer.mm suppresses CAOpenGLLayer/CGL deprecation.
...
#pragma clang diagnostic pop
  ...
  g_signal_connect(window->area, "button-press-event", G_CALLBACK(on_button_press_event), window);
  ...
  g_signal_connect(window->area, "motion-notify-event", G_CALLBACK(on_motion_notify_event),
                   window);
  g_signal_connect(window->area, "leave-notify-event", G_CALLBACK(on_leave_notify_event), window);
  ...
  // Manual target list rather than gtk_drag_dest_add_text_targets: only "text/uri-list" is wanted
  // (file drops), which is the one URI-specific target ST's confirmed symbol set does not have a
  // one-line convenience call for -- gtk_target_list_new/_add/_table_new_from_list are.
  ...
  g_signal_connect(window->area, "drag-data-received", G_CALLBACK(on_drag_data_received), window);
...
void px_destroy_window(px_window_t* window) {
  ...
  if (window->window) {
    gtk_widget_destroy(window->window);  // triggers on_destroy for the PX_EVENT_DESTROY dispatch
...
void px_close_window(px_window_t* window) {
  if (window && window->window) {
    // Runs the same delete-event -> can_close_without_prompt()/try_close() path a user clicking
    // the window's own close button would, the asynchronous analogue of PostMessageW(WM_CLOSE).
...
void px_reset_cursor(px_window_t* window) {
  ...
  px_set_cursor(window,
               window->handler->calculate_cursor(vec2{static_cast<double>(x), static_cast<double>(y)}));
}
