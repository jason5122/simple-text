...
void px_init(const char* app_name, const char* bundle_id, int argc, char** argv, uint32_t flags) {
  ...
  // Instantiating the shared application through our subclass is what makes NSApp a PXApplication;
  // there is no Info.plist NSPrincipalClass in play here.
  ...
  // silently ignored until -run. Calling -run after an explicit -finishLaunching is supported and
  // simply enters the already-initialised application event loop.
...
void px_set_timeout(std::function<void()> fn, int milliseconds) {
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, static_cast<int64_t>(milliseconds) * NSEC_PER_MSEC),
...
void px_show_error(px_window_t* parent, const char* message) {
  ...
  if (parent && parent->window) {
    [alert beginSheetModalForWindow:parent->window completionHandler:^(NSModalResponse){
    }];
  } else {
    [alert runModal];
  }
