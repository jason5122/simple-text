...
struct px_window_t {
  ...
  // Set the first time the layer's draw callback runs. send_event gates its repaint flush on this
  // (ST reads the same flag at px_window_t+0x38, which drawInCGLContext: sets to 1), so nothing
  // tries to flush before the GL drawable exists.
  ...
  // clock and CVTimeStamp.hostTime are both based on system uptime, so they can be compared without
  // involving wall-clock time. The serial is published last and acquired by the render thread.
