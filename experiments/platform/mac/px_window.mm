...
NSCursor* ns_cursor(px_cursor_t cursor) {
  switch (cursor) {
    case PX_CURSOR_IBEAM: return [NSCursor IBeamCursor];
    case PX_CURSOR_CROSSHAIR: return [NSCursor crosshairCursor];
    case PX_CURSOR_POINTING_HAND: return [NSCursor pointingHandCursor];
    case PX_CURSOR_RESIZE_LEFT_RIGHT: return [NSCursor resizeLeftRightCursor];
    case PX_CURSOR_RESIZE_UP_DOWN: return [NSCursor resizeUpDownCursor];
...
- (void)drawRect:(NSRect)dirtyRect {
  ...
  // -[PXView softwareDrawWithRects:width:height:...] and a px_canvas; no CPU rasteriser exists in
  // this experiment, so this only clears to the window background.
  if (!_pxw) {
    return;
  }
...
- (void)resetCursorRects {
  ...
  // ST asks the handler per-point via calculate_cursor. Cocoa wants rects, so the whole view gets
  // the cursor for the current mouse location and the handler is re-asked on every motion.
  const NSPoint p = [self convertPoint:self.window.mouseLocationOutsideOfEventStream fromView:nil];
...
- (void)scrollWheel:(NSEvent*)event {
  ...
  if (e.precise_scroll) {
    ...
    e.scroll_delta = vec2{event.scrollingDeltaX * kLineHeight, event.scrollingDeltaY * kLineHeight};
  }
  px_mac_send_event(_pxw, &e);
}
...
void px_mac_flush_dirty_rects(px_window_t* window) {
  ...
  for (const rect& r : window->dirty) {
    if (window->use_gl) {
      ...
      // CAOpenGLLayer does not clip to the invalidated sub-rect, so this only marks the layer as
      // needing display; the authoritative region list is the one handed to paint() below.
...
void px_mac_dispatch_post_event_callbacks() {
  ...
  // Swap out first: a callback is allowed to enqueue another one, and that one belongs to the next
  // drain, not this loop.
...
void update_cursor_from_tracking_rects(px_window_t* window) {
  ...
  const NSPoint p =
      [window->view convertPoint:window->window.mouseLocationOutsideOfEventStream fromView:nil];
...
void px_mac_install_before_waiting_observer() {
  ...
  CFRunLoopObserverRef observer =
      CFRunLoopObserverCreate(kCFAllocatorDefault, kCFRunLoopBeforeWaiting, true, 0,
                              &before_waiting_callback, &context);
  if (!observer) {
    return;
  }
...
CVReturn display_link_callback(CVDisplayLinkRef link,
  ...
  // The link fires on its own thread. Coalesce so a slow main thread cannot accumulate a backlog.
  bool expected = false;
...
void px_mac_update_display_link(px_window_t* window) {
  ...
  if (screen_number) {
    CVDisplayLinkSetCurrentCGDisplay(window->display_link,
...
void px_show_window(px_window_t* window) {
  ...
  // from the currently active process. This demo calls show() before NSApp enters -run, so do the
  // activation immediately and repeat it on the first main-queue turn after launch. The second
  // call is harmless for windows shown later while the event loop is already running.
  const auto activate = [] {
...
vec2 px_window_position(px_window_t* window) {
  ...
  // Sublime reports the content rect's top-left, rather than the outer frame's top-left. This lets
  // controls turn an event-local point into screen space by simple addition.
...
void px_set_window_position(px_window_t* window, vec2 position) {
  ...
  const NSRect frame = window->window.frame;
  const NSRect content = [window->window contentRectForFrameRect:frame];
  [window->window setFrameOrigin:NSMakePoint(position.x,
                                             screen_height - position.y - content.size.height)];
}
