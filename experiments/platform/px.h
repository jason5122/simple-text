...
enum px_event_type : int {
  // 0 is the value a memset leaves behind, and it is what ST's key path relies on: the WM_KEYDOWN
...
// control::handle_event takes `px_event_t const*`, so it is never rewrapped into an app-level type.
//
...
// RENDER CONTEXT
// ─────────────────────────────────────────────────────────────────────────────────────────────────
// Handed to px_window_event_handler::paint. ST's px_render_context is a batching 2D renderer
// (begin_rect_batch / draw_text / end_text_batch); this one deliberately stops at the platform
// boundary and just guarantees the GL state a drawing layer needs, because a renderer is not part
// of the abstraction being compared.
//
// On entry to paint(): the shared GL context is current, the layer's framebuffer is bound, and the
// viewport covers the whole drawable in device pixels.
class px_render_context {
 public:
  virtual ~px_render_context() = default;

  // Drawable size in device pixels.
  virtual vec2 device_size() const = 0;

  // Points -> device pixels.
  virtual double dpi_scale_factor() const = 0;

  // Converts a window-space rect (points, top-left origin) to a GL scissor box (device pixels,
  // bottom-left origin), clamped to the drawable.
  virtual void scissor_box(rect r, int out_xywh[4]) const = 0;
};
...
class px_input_client {
 public:
  ...
  virtual void set_marked_text(const char* utf8, px_range_t selected, px_range_t replacement) = 0;
...
enum : uint32_t {
  ...
  // helper omits its 0x20000 OpenGL-enable flag, so its cached texture window takes this path even
  // though the editor's main windows use GL.
