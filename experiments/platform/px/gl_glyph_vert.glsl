...
void main() {
    ...
    glyph_color = texelFetch(instances, instance_index * 3 + 0);
    vec4 dst = texelFetch(instances, instance_index * 3 + 1);
    vec4 texture_source = texelFetch(instances, instance_index * 3 + 2);
    ...
    vec2 texture_position = texture_source.xy + corner * dst.zw;
