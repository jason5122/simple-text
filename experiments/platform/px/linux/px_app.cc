// Process-level half of the GTK3 backend: init, the main loop, timers, and the odds and ends of
// the flat API that are not per-window.
//
// ST calls the ordinary blocking gtk_main() -- gtk_main, gtk_main_level and gtk_main_quit are all
// in the resolved symbol list, and nothing suggests a hand-rolled GMainLoop replacement. That is
// at odds with pre_sleep() needing a hook at the moment the loop is about to block, until the rest
// of the import table is read: ST also links g_source_new, g_source_attach and g_source_set_name
...
gboolean pre_sleep_prepare(GSource*, gint* timeout) {
    ...
    // Never itself ready to dispatch; only its side effect (the flush above) matters. -1 imposes
    // no timeout of its own, deferring to whatever other source has the nearest deadline.
    *timeout = -1;
    return FALSE;
}

gboolean pre_sleep_check(GSource*) { return FALSE; }

gboolean pre_sleep_dispatch(GSource*, GSourceFunc, gpointer) { return G_SOURCE_CONTINUE; }
...
void px_run_event_loop() { gtk_main(); }
...
bool px_os_in_dark_mode() {
    ...
    // Cross-checked against the theme name, the way ST watches notify::gtk-theme-name: a theme
    // named e.g. "Adwaita-dark" can be dark without the application-level preference being set.
...
double px_caret_blink_time() {
    ...
    gint blink_ms = 1200;
    g_object_get(settings, "gtk-cursor-blink", &blink, "gtk-cursor-blink-time", &blink_ms,
                 nullptr);
    if (!blink || blink_ms <= 0) {
        return 0.0;  // "do not blink"
    }
    // gtk-cursor-blink-time is a full on+off cycle, matching the "seconds per phase" contract
    // px_caret_blink_time documents the same way ST's macOS backend averages
    // NSTextInsertionPoint's two phase durations.
    return blink_ms / 2000.0;
}

void px_show_error(px_window_t* parent, const char* message) {
    GtkWidget* dialog =
        gtk_message_dialog_new(parent ? GTK_WINDOW(parent->window) : nullptr, GTK_DIALOG_MODAL,
                               GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s", message ? message : "");
...
void px_open_url(const char* url) {
    ...
    // gtk_show_uri is the confirmed symbol ST resolves -- not gtk_show_uri_on_window, its
    // GTK 3.22+ replacement, which needs a parent GtkWindow this API does not have on hand. Newer
    // GTK headers deprecate it; suppressed for the same reason mac/px_gl_layer.mm suppresses
    // CAOpenGLLayer/CGL deprecation warnings -- ST 4200 ships on the older call, and mirroring
    // that is the point.
