...
void px_run_event_loop() {
    ...
    while (!g_quit) {
        ...
        // The queue is empty. Give every window a chance to settle before the thread blocks: this
        // is what pre_sleep() exists for, and why the loop cannot be GetMessageW. Iterated over a
        // copy, because a handler is allowed to destroy a window from here.
...
void px_set_timeout(std::function<void()> fn, int milliseconds) {
    const UINT_PTR id =
        SetTimer(nullptr, g_next_timer_id++, static_cast<UINT>(milliseconds), &timer_proc);
    if (id != 0) {
