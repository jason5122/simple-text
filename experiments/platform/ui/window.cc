...
window_impl::window_impl(double width, double height, const char* title, fcolor background) {
    ...
    // argument.
    px_window_ =
        px_create_window(this, nullptr, width, height, title, background, PX_WINDOW_DEFAULT);
}
...
void window_impl::show() { px_show_window(px_window_); }

void window_impl::hide() { px_hide_window(px_window_); }

void window_impl::close() { px_close_window(px_window_); }
...
void window_impl::mark_dirty() { px_mark_dirty(px_window_); }
...
void window_impl::set_root_control(control* root) { root_ = root; }

void window_impl::set_handler(window_handler* handler) { handler_ = handler; }

void window_impl::set_input_client(px_input_client* client) { input_client_ = client; }

// ── px_window_event_handler ─────────────────────────────────────────────────────────────────────

bool window_impl::handle_event(px_event_t* event) {
    // The chain, transcribed from 0x140197e22. Note there is no switch on event->type here:
    // routing by tag is each aspect's business, and the control tree's.
    bool handled = false;
...
void window_impl::paint(px_render_context* rc, rect bounds, const rect* dirty, int dirty_count) {
    if (root_) {
        ...
        // object, so recover the equivalent full root bounds from the window here.
        const vec2 root_size = size();
        root_->draw(rc, rect{0.0, 0.0, root_size.x, root_size.y}, dirty, dirty_count);
    }
}
...
void window_basic_aspect::notify_event(px_event_t* event) { observe(event); }
...
bool window_hover_aspect::handle_event(px_event_t* event) {
    ...
    // -- the same division of labor ST's drag_resizer relies on, marking only its own control
    // dirty rather than reaching for a window-wide invalidation on every mouse-move sample.
    return false;
}

void window_hover_aspect::notify_event(px_event_t* event) { observe(event); }
