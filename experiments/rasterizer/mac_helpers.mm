...
void blit_pixels(ScopedCGContext ctx, const BitmapView& img, double x, double y) {
    ...
    const double surface_height = CGBitmapContextGetHeight(ctx.get());
    ...
    CGContextDrawImage(ctx.get(), dst, cgimg.get());
    CGContextRestoreGState(ctx.get());
}

void show_window(ScopedCGContext ctx, double scale) {
    auto image = ScopedCGImage(CGBitmapContextCreateImage(ctx.get()));
