...
// the Windows build disassembled under reverse_engineering/ (sublime_text.exe, build 4200 x64).
//
...
const Globals& globals() {
    static const Globals g = [] {
        ...
        if (result.factory) {
            ...
            // Sublime builds one IDWriteRenderingParams per monitor
            // (EnumDisplayMonitors -> CreateMonitorRenderingParams, 0x1401cb62e) and indexes them
            // by monitor. Slot 0 is the process default, which is all an offscreen rasterizer
            // needs.
...
struct FontHandle::FontData {
    ...
    // a separate ceil of the whole sum rather than ascent + descent.
    float ascent = 0;
    float descent = 0;
    float line_height = 0;
...
// Sublime derives grayscale and aliased rendering from the monitor's params by zeroing the
// ClearType level and flattening the pixel geometry (0x1401bb9b8), rather than by asking for a
...
ComPtr<IDWriteRenderingParams> params_for(uint32_t flags) {
    ...
    // Sublime keeps the monitor's gamma and contrast, because it draws the glyph in its final
    // colour and hands the result straight to the compositor. We draw white on black and reuse
    // that as a coverage mask, then paint dark text on a light background -- the opposite
    // polarity, so DirectWrite's correction is applied the wrong way round and thickens the
    // antialiased ramp. Measured against Sublime it puts edge pixels ~27 levels too dark while
    // stroke cores match to within 1. Neutralising both gets the mask close to linear coverage;
    // drawing in the final colour, the way Sublime does, is the real fix.
    constexpr FLOAT kMaskGamma = 1.0f;
    constexpr FLOAT kMaskContrast = 0.0f;
    const FLOAT gamma = g_debug_gamma > 0 ? g_debug_gamma : kMaskGamma;
    const FLOAT contrast = g_debug_gamma > 0 ? g_debug_contrast : kMaskContrast;

    ComPtr<IDWriteRenderingParams> custom;
    if (FAILED(g.factory->CreateCustomRenderingParams(
            gamma, contrast, grayscale ? 0.0f : g.rendering_params->GetClearTypeLevel(),
...
// Packs one BGRA pixel the way the atlas wants it: coverage in alpha, rgb zeroed. DirectWrite
// hands back a glyph painted in `color` on black, so any channel is the coverage; averaging the
// three is what Sublime does when it needs a single number (0x1401bc032).
// DirectWrite paints the glyph over black, so each channel already holds that subpixel's coverage
// (mono) or its premultiplied colour (a colour run). Either way the channels pass through
// untouched and alpha becomes their mean -- which is Sublime's rule for colour runs, and gives a
// renderer without dual-source blending something usable for mono ones.
...
// ceil of ascent + descent + lineGap rather than the sum of the two rounded values. Reporting the
// difference here makes a caller's ceil(ascent) + ceil(descent) + ceil(leading) land exactly on
// Sublime's line height.
...
std::optional<FontHandle> create_font(std::string family,
    ...
    data->ascent = std::ceil(static_cast<float>(metrics.ascent + metrics.lineGap) * upem_scale);
    data->descent = std::ceil(static_cast<float>(metrics.descent) * upem_scale);
    data->line_height = std::ceil(
...
GlyphBitmap rasterize(const FontHandle& font, GlyphId glyph, double scale, double subpixel_x) {
    ...
    if (!use_analysis_path() && ensure_target(data, tile->width, tile->height, scale)) {
        ...
        if (colored) {
            for (;;) {
                BOOL has_run = FALSE;
                if (FAILED(color_layers->MoveNext(&has_run)) || !has_run) break;
                const DWRITE_COLOR_GLYPH_RUN* layer = nullptr;
                if (FAILED(color_layers->GetCurrentRun(&layer))) break;
                const COLORREF color = RGB(base::clamp_round<int>(layer->runColor.r * 255.0f),
                                           base::clamp_round<int>(layer->runColor.g * 255.0f),
                                           base::clamp_round<int>(layer->runColor.b * 255.0f));
