...
EditorWidget::EditorWidget(size_t font_id, int font_size)
    : font_id(font_id),
      font_size(font_size),
      multi_view{new MultiViewWidget<TextViewWidget>{}},
      tab_bar{new TabBarWidget{kTabBarHeight}} {
    ...
    multi_view->addTab(std::make_shared<TextViewWidget>("", font_id, font_size));
...
void EditorWidget::addTab(std::string_view tab_name, std::string_view text) {
    multi_view->addTab(std::make_shared<TextViewWidget>(text, font_id, font_size));
