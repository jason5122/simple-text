...
gboolean on_draw(GtkWidget* widget, cairo_t* cr, gpointer data) {
    ...
    if (GdkScreen* screen = gdk_screen_get_default()) {
        if (const cairo_font_options_t* options = gdk_screen_get_font_options(screen)) {
            subpixel_order = static_cast<uint32_t>(cairo_font_options_get_subpixel_order(options));
        }
    }
    ...
    if (window->use_gl && px_linux_gl_make_current(window) &&
        ...
        if (device_w > 0 && device_h > 0 &&
            static_cast<size_t>(device_w) <= std::numeric_limits<size_t>::max() /
                                                 static_cast<size_t>(device_h) /
                                                 sizeof(uint32_t)) {
