...
RedBlackTree::RedBlackTree(Color c, const RedBlackTree& lft, const NodeData& val,
...
RedBlackTree RedBlackTree::ins(const NodeData& x, Offset at, Offset total_offset) const {
    if (is_empty()) return RedBlackTree(Color::Red, RedBlackTree(), x, RedBlackTree());
    ...
    if (at < total_offset + y.left_subtree_length + y.piece.length) {
        ...
        return balance(root_color(), left(), y,
                       right().ins(x, at, total_offset + y.left_subtree_length + y.piece.length));
    }
}

RedBlackTree RedBlackTree::balance(Color c, const RedBlackTree& lft, const NodeData& x,
                                   const RedBlackTree& rgt) {
...
bool RedBlackTree::doubled_left() const {
    return not is_empty() and root_color() == Color::Red and not left().is_empty() and
           left().root_color() == Color::Red;
}

bool RedBlackTree::doubled_right() const {
    return not is_empty() and root_color() == Color::Red and not right().is_empty() and
...
struct RedBlackTree::ColorTree {
    ...
    explicit ColorTree(Color c, const RedBlackTree& lft, const NodeData& x,
                       const RedBlackTree& rgt)
...
RedBlackTree RedBlackTree::remove(Offset at) const {
    ...
    if (t.is_empty()) return RedBlackTree();
    return RedBlackTree(Color::Black, t.left(), t.root(), t.right());
}
...
RedBlackTree RedBlackTree::balance(const RedBlackTree& node) {
    // Two red children.
    if (not node.left().is_empty() and node.left().root_color() == Color::Red and
        not node.right().is_empty() and node.right().root_color() == Color::Red) {
...
RedBlackTree RedBlackTree::balance_left(const RedBlackTree& left) {
    ...
    if (not left.right().is_empty() and left.right().root_color() == Color::Red and
        not left.right().left().is_empty() and left.right().left().root_color() == Color::Black) {
...
RedBlackTree RedBlackTree::balance_right(const RedBlackTree& right) {
    ...
    if (not right.left().is_empty() and right.left().root_color() == Color::Red and
        not right.left().right().is_empty() and
...
RedBlackTree RedBlackTree::remove_left(const RedBlackTree& root, Offset at, Offset total) {
    ...
    if (not root.left().is_empty() and root.left().root_color() == Color::Black)
        return balance_left(new_node);
    return new_node;
}
...
void Tree::build_tree() {
    ...
    for (size_t i = 0; i < buf_count; ++i) {
        ...
        Piece piece{
            ...
            .last = {.line = last_line,
                     .column = Column{buf.buffer.size() - rep(buf.line_starts[rep(last_line)])}},
...
void Tree::internal_insert(CharOffset offset, std::string_view txt) {
    ...
    if (node_start_offset == offset) {
        ...
        if (offset != CharOffset{}) {
            ...
            if (prev_node_result.node->piece.index == BufferIndex::ModBuf and
...
// Fetches the length of the piece starting from the first line to 'index' or to the end of
// the piece.
Length Tree::accumulate_value_no_lf(const BufferCollection* buffers, const Piece& piece,
                                    Line index) {
    auto* buffer = buffers->buffer_at(piece.index);
    auto& line_starts = buffer->line_starts;
    // Extend it so we can capture the entire line content including newline.
...
void Tree::populate_from_node(std::string* buf, const BufferCollection* buffers,
    ...
    auto accumulated_value =
        accumulate_value(buffers, node.root().piece, node.root().piece.first.line);
...
void Tree::populate_from_node(std::string* buf, const BufferCollection* buffers,
                              const PieceTree::RedBlackTree& node, Line line_index) {
...
void Tree::line_start(CharOffset* offset, const BufferCollection* buffers,
                      const PieceTree::RedBlackTree& node, Line line) {
    ...
    else if (rep(node.root().left_subtree_lf_count + node.root().piece.newline_count) >=
             line_index) {
...
void Tree::line_end_crlf(CharOffset* offset, const BufferCollection* buffers,
                         const RedBlackTree& root, const RedBlackTree& node, Line line) {
    if (node.is_empty()) return;
    assert(line != Line::IndexBeginning);
    auto line_index = rep(retract(line));
    if (rep(node.root().left_subtree_lf_count) >= line_index) {
    ...
    // The desired line is directly within the node.
        ...
        // If the length is anything but 0, we need to check if the last character was a carriage
        // return.
        if (len != Length{}) {
            auto last_char_offset = *offset + retract(len);
            if (char_at(buffers, root, last_char_offset) == '\r' and
                char_at(buffers, root, extend(last_char_offset)) == '\n') {
                len = retract(len);
            }
        }
        *offset = *offset + len;
    ...
    // assemble the LHS and RHS.
        ...
        // This case implies that 'left_subtree_lf_count + piece NL count' is strictly <
        // line_index. The content is somewhere in the middle.
...
Line Tree::line_at(CharOffset offset) const {
    if (is_empty()) return Line::Beginning;
    auto result = node_at(&buffers, root, offset);
    return result.line;
}
...
void Tree::get_line_content(std::string* buf, Line line) const {
    // Reset the buffer.
    buf->clear();
    if (line == Line::IndexBeginning) return;
...
IncompleteCRLF Tree::get_line_content_crlf(std::string* buf, Line line) const {
    // Reset the buffer.
    buf->clear();
    if (line == Line::IndexBeginning) return IncompleteCRLF::No;
    auto node = root;
    if (node.is_empty()) return IncompleteCRLF::No;
...
LFCount Tree::line_feed_count(const BufferCollection* buffers, BufferIndex index,
                              const BufferCursor& start, const BufferCursor& end) {
    // If the end position is the beginning of a new line, then we can just return the difference
    // in lines.
    if (rep(end.column) == 0) return LFCount{rep(retract(end.line, rep(start.line)))};
...
Piece Tree::build_piece(std::string_view txt) {
    ...
    Piece piece = {.index = BufferIndex::ModBuf,
                   ...
                   .newline_count =
                       line_feed_count(&buffers, BufferIndex::ModBuf, start, end_pos)};
    ...
    while (not node.is_empty()) {
        if (rep(node.root().left_subtree_length) > rep(off)) {
            ...
            if (node.right().is_empty()) {
                ...
                newline_count +=
                    rep(node.root().left_subtree_lf_count + node.root().piece.newline_count);
                // Now we find the line within this piece.
...
BufferCursor Tree::buffer_position(const BufferCollection* buffers, const Piece& piece,
                                   Length remainder) {
    ...
    return {.line = Line{mid}, .column = Column{offset - mid_start}};
}

Piece Tree::trim_piece_right(const BufferCollection* buffers, const Piece& piece,
...
Piece Tree::trim_piece_left(const BufferCollection* buffers, const Piece& piece,
                            const BufferCursor& pos) {
...
Tree::ShrinkResult Tree::shrink_piece(const BufferCollection* buffers, const Piece& piece,
                                      const BufferCursor& first, const BufferCursor& last) {
...
void Tree::insert(CharOffset offset, std::string_view txt, SuppressHistory suppress_history) {
    ...
    if (is_no(suppress_history) and (end_last_insert != offset or root.is_empty())) {
...
void Tree::remove(CharOffset offset, Length count, SuppressHistory suppress_history) {
    ...
    if (rep(count) == 0 or root.is_empty()) return;
...
UndoRedoResult Tree::try_undo(CharOffset op_offset) {
    if (undo_stack.empty()) return {.success = false, .op_offset = CharOffset{}};
...
UndoRedoResult Tree::try_redo(CharOffset op_offset) {
    if (redo_stack.empty()) return {.success = false, .op_offset = CharOffset{}};
...
OwningSnapshot::OwningSnapshot(const Tree* tree)
    : root{tree->root}, meta{tree->meta}, buffers{tree->buffers} {}
...
ReferenceSnapshot::ReferenceSnapshot(const Tree* tree, const RedBlackTree& dt)
    : root{dt}, meta{tree->meta}, buffers{&tree->buffers} {
    // Compute the buffer meta for 'dt'.
    compute_buffer_meta(&meta, dt);
}
...
bool TreeWalker::exhausted() const {
    ...
    if (entry.node.is_empty()) return true;
    if (entry.dir == Direction::Right and entry.node.right().is_empty()) return true;
    return false;
}
...
bool ReverseTreeWalker::exhausted() const {
    ...
    if (entry.dir == Direction::Left and entry.node.left().is_empty()) return true;
    return false;
}
