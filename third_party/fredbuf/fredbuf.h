...
public:
    ...
    // This will commit the current node to the history.  The offset provided will be the undo
    // point later.
    void commit_head(CharOffset offset);
    RedBlackTree head() const;
    // Snaps the tree back to the specified root.  This needs to be called with a root that is
    // derived from the set of buffers based on its creation.
...
private:
    ...
    static void line_start(CharOffset* offset, const BufferCollection* buffers,
                           const RedBlackTree& node, Line line);
    static void line_end_crlf(CharOffset* offset, const BufferCollection* buffers,
                              const RedBlackTree& root, const RedBlackTree& node, Line line);
    static Length accumulate_value(const BufferCollection* buffers, const Piece& piece,
                                   Line index);
    static Length accumulate_value_no_lf(const BufferCollection* buffers, const Piece& piece,
                                         Line index);
    static void populate_from_node(std::string* buf, const BufferCollection* buffers,
                                   const RedBlackTree& node);
    static void populate_from_node(std::string* buf, const BufferCollection* buffers,
                                   const RedBlackTree& node, Line line_index);
    static LFCount line_feed_count(const BufferCollection* buffers, BufferIndex index,
                                   const BufferCursor& start, const BufferCursor& end);
    static NodePosition node_at(const BufferCollection* buffers, RedBlackTree node,
                                CharOffset off);
    static BufferCursor buffer_position(const BufferCollection* buffers, const Piece& piece,
                                        Length remainder);
    static char char_at(const BufferCollection* buffers, const RedBlackTree& node,
                        CharOffset offset);
    static Piece trim_piece_right(const BufferCollection* buffers, const Piece& piece,
                                  const BufferCursor& pos);
    static Piece trim_piece_left(const BufferCollection* buffers, const Piece& piece,
                                 const BufferCursor& pos);
    ...
    static ShrinkResult shrink_piece(const BufferCollection* buffers, const Piece& piece,
