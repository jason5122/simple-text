...
def main(argv):
    ...
    parser.add_argument(
        'protocols', nargs='+', help='Input protocol file paths relative to src root.'
    )
    parser.add_argument(
        '--generator-type',
        help='Controls what type of\
                      headers/code shall be generated',
        default='all',
        choices={'all', 'protocol-client', 'protocol-server', 'protocol-marshalling'},
    )
    ...
    for protocol in protocols:
        ...
        if generator_type == 'protocol-marshalling':
            generate_code(
                cmd, [private_code_type], protocol_path, out_base_name + '-protocol.c'
            )
        elif generator_type == 'protocol-client':
            scanner_args = ['client-header']
            generate_code(
                cmd, scanner_args, protocol_path, out_base_name + '-client-protocol.h'
            )
            ...
            generate_code(
                cmd, scanner_args, protocol_path, out_base_name + '-server-protocol.h'
            )
            scanner_args.append('-c')
            generate_code(
                cmd,
