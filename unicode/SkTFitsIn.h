...
/**
 * In C++ an unsigned to signed cast where the source value cannot be represented in the
 * destination type results in an implementation defined destination value. Unlike C, C++ does not
 * allow a trap. This makes "(S)(D)s == s" a possibly useful test. However, there are two cases
 ...
 *                             => true
 *
 * So for the eight cases three are trivially true, three more are valid casts, and two are
 * special. The two 'full' checks which otherwise require two comparisons are valid cast checks.
